class Mentor < ActiveRecord::Base
  ...
  def get_possible_mentors
    User.where(is_participating_next_month: true, mentor: true, waitlist: false).where("stage_of_career > ? AND mentor_times > ?", current_user.stage_of_career, 0).where(mentor_industry: current_user.primary_industry)
  end
